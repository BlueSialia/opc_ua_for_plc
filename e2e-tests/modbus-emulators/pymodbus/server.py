...
Sets up known holding-register and coil values that the gateway's test
config expects, then runs forever so the gateway can poll it.
"""
...
def build_context():
    ...
    holding = ModbusSequentialDataBlock(
        1,
        [
            0xABCD,  # register 1 — UInt16 (address 0 in gateway)
            0x0000,  # register 2 — padding
            0x1234,  # register 3 — UInt32 hi word (address 2 in gateway)
            0x5678,  # register 4 — UInt32 lo word
            (pi_bits >> 16)
            & 0xFFFF,  # register 5 — Float hi word (address 4 in gateway)
...
if __name__ == "__main__":
    ...
    identity.VendorName = "opc-ua-for-plc e2e"
    identity.ProductCode = "E2E"
    identity.VendorUrl = "https://github.com/BlueSialia/opc_ua_for_plc"
